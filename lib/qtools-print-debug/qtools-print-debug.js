...
var moduleFunction = function(args) {
	...
	const addToPrototype = () => (commonFunctions) => {
	
		function qtDump(args={}) {
			const {label='', returnString=false, addSeparators=false}=args
			const inData = this;
			const callerName = commonFunctions.callerName(true);
			const separator=addSeparators?'\n--------\n':'';
			const returnn=addSeparators?'\n':' ';
			...
			if (['string', 'number', 'boolean'].includes(typeof this)) {
				if (returnString) {
					...
					console.log(
						`${separator}${label ? showLabel + '= ' : ''}${this} [${callerName}]${separator}`
					);
				}
				...
				if (returnString) {
					...
					console.log(
						`${separator}${label ? showLabel + `=${returnn}` : ''}${returnn}${this}${returnn}${returnn}[${callerName}]${separator}`
					);
				}
			} else {
				...
				if (returnString) {
					return util.inspect(tmpObject, { depth: null, maxArrayLength: null })+` [${callerName}]`;
				} else {
					console.log(
						`${separator}${label ? showLabel + `:${returnn}` : ''}${util.inspect(inData, {
							depth: null
		...
		return {
			methods: ['any.qtDump({label:\'\', returnString:false, addSeparators=false})', 
